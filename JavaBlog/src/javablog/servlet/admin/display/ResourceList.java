...
public class ResourceList extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			if (this.isValidStatus(status)
				...
				if (connection != null){
					...
					if (ub != null && ub.getStatus() == ConfigProperty.STATUS_NORMAL){
						...
						if (role != null && role.getStatus() == 0 && role.canReadResource()) {
							ResourceBeanBo rbb = new ResourceBeanBo(connection);
							rbb.setPageSize(ConfigProperty.admin_resource_page_size);
							rbb.setFilter(status, attribute, order);
							ArrayList<ResourceBean> al = rbb.getResources(pageNow);
							request.setAttribute("resourceList", al);
							request.setAttribute("role", role);
							request.setAttribute("pageNow", pageNow + "");
							request.setAttribute("rowCount", rbb.getRowCount() + "");
							request.setAttribute("pageCount", rbb.getPageCount() + "");
		...
		if (flag){
			request.getRequestDispatcher("resourceList.jsp").forward(request,
					response);
		} else {
	...
	private boolean isValidAttribute(String attribute){
		...
		if (attribute == null
				|| attribute.length() == 0
				|| attribute.equalsIgnoreCase("resource_id")
