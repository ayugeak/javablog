...
public class TagList extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			if (this.isValidStatus(status)
				...
				if (connection != null){
					...
					if (ub != null && ub.getStatus() == ConfigProperty.STATUS_NORMAL){
						...
						if (role != null && role.getStatus() == 0 && role.canReadTag()) {
							TagBeanBo tbb = new TagBeanBo(connection);
							tbb.setPageSize(ConfigProperty.admin_tag_page_size);
							tbb.setFilter(status, attribute, order);
							ArrayList<TagBean> al = tbb.getTags(pageNow);
							request.setAttribute("tagList", al);
							request.setAttribute("role", role);
							request.setAttribute("pageNow", pageNow + "");
							request.setAttribute("rowCount", tbb.getRowCount() + "");
							request.setAttribute("pageCount", tbb.getPageCount() + "");
		...
		if (flag){
			request.getRequestDispatcher("tagList.jsp").forward(request,
					response);
		} else {
	...
	private boolean isValidAttribute(String attribute){
		...
		if (attribute == null
				|| attribute.length() == 0
				|| attribute.equalsIgnoreCase("tag_id")
