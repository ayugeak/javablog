...
public class RoleList extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			if (this.isValidStatus(status)
				...
				if (connection != null){
					...
					if (ub != null && ub.getStatus() == ConfigProperty.STATUS_NORMAL){
						...
						if (role != null && role.getStatus() == 0 && role.canReadRole()) {
							rbb.setPageSize(ConfigProperty.admin_role_page_size);
							rbb.setFilter(status, attribute, order);
							ArrayList<RoleBean> al = rbb.getRoles(pageNow);
			
							request.setAttribute("roleList", al);
		...
		if (flag){
			request.getRequestDispatcher("roleList.jsp").forward(request,
					response);
		} else {
