...
public class LinkList extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			if (this.isValidStatus(status)
				...
				if (connection != null){
					...
					if (ub != null && ub.getStatus() == ConfigProperty.STATUS_NORMAL){
						...
						if (role != null && role.getStatus() == 0 && role.canReadLink()) {
							LinkBeanBo lbb = new LinkBeanBo(connection);
							lbb.setPageSize(ConfigProperty.admin_link_page_size);
							lbb.setFilter(status, attribute, order);
							ArrayList<LinkBean> al = lbb.getLinks(pageNow);
			
							request.setAttribute("linkList", al);
		...
		if (flag){
			request.getRequestDispatcher("linkList.jsp").forward(request,
					response);
		} else {
	...
	private boolean isValidAttribute(String attribute){
		...
		if (attribute == null
				|| attribute.length() == 0
				|| attribute.equalsIgnoreCase("link_id")
				|| attribute.equalsIgnoreCase("name")
				|| attribute.equalsIgnoreCase("url")){
			result = true;
		}
		return result;
	}
