	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (userIdText != null) {
				...
				int tagId = sf.parseNum(tagIdText);
				...
				if (connection != null){
					...
					if (ub != null && ub.getStatus() == ConfigProperty.STATUS_NORMAL){
						...
						if (role != null && role.getStatus() == ConfigProperty.STATUS_NORMAL){
							...
							tb.setTagId(tagId);
							tb.setName(sf.htmlFilter(name));
							tb.setStatus(ConfigProperty.STATUS_NORMAL);
			
							TagBeanBo tbb = null;// 标签管理类
							String content = null;// 日志内容
							// 判断用户是否具有相应权限
							if (tagId == 0 && role.canAddTag()) {// 新建标签
								tbb = new TagBeanBo(connection);
								if (tbb.addTag(tb)) {
									result = "s";
									content = ub.getName() + "(" + ub.getUsername()
											+ ")添加了标签:" + name;
								}
							} else if (tagId != 0 && role.canEditTag()) {// 修改标签
								tbb = new TagBeanBo(connection);
								if (tbb.editTag(tb)) {
									result = "s";
									content = ub.getName() + "(" + ub.getUsername()
